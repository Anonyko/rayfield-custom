...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if (Settings.a2FuZ2VuIGdyZXMgMjEwNA) then
		...
		if typeof(Settings.KeySettings.Key) == "string" then Settings.KeySettings.Key = {Settings.KeySettings.Key} end
            ...
            local Success, Response = pcall(function()
                ...
                local response = game:HttpGet(KeySettings.Key)
                -- Split the response into lines and store in a table
                local keys = {}
                for line in string.gmatch(response, "[^\n\r]+") do
                    table.insert(keys, line)
                end
                -- Replace KeySettings.Key with the processed array of keys
                KeySettings.ProcessedKeys = keys -- Use a separate field to store the array
            end)
		...
		if isfile and isfile(RayfieldFolder .. "/Key System" .. "/" .. Settings.KeySettings.FileName .. ConfigurationExtension) then
            if type(Settings.KeySettings.Key) == "table" then
                for _, MKey in ipairs(Settings.KeySettings.Key) do
                    local fileContent = readfile(RayfieldFolder .. "/Key System" .. "/" .. Settings.KeySettings.FileName .. ConfigurationExtension)
                    if string.find(fileContent, MKey) then
                        Passthrough = true
...
if useStudio then
	...
	local Window = RayfieldLibrary:CreateWindow({
		...
		a2FuZ2VuIGdyZXMgMjEwNA = false, -- Set this to true to use our key system
		KeySettings = {
			Title = "Untitled",
			Subtitle = "Key System",
